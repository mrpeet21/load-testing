...
среде с использованием средств нагрузочного тестирования HammerDB<br>  
...
- **Гипервизор:** Oracle VirtualBox  7.2.6
- **СУБД:** PostgreSQL 16.13  
- **Гостевые ОС:**
  - Ubuntu 24.04 LTS
  - Windows 11  
- **Инструмент нагрузки:** HammerDB 5.0
...
Для повышения достоверности результатов каждая ОС тестировалась по 3 раза, после чего рассчитывались средние значения.
...
В ходе работы было проведено исследование производительности PostgreSQL в виртуализированной среде с использованием Oracle VirtualBox и HammerDB. Были развернуты две гостевые ОС — **Ubuntu 24.04 LTS** и **Windows 11**, установлена PostgreSQL 16, настроено сетевое подключение к СУБД и выполнено нагрузочное тестирование по сценарию **TPROC-C**.

На первом этапе были выбраны основные инструменты и задана базовая конфигурация виртуальных машин: **2 vCPU, 4 ГБ RAM, 40 ГБ диска**. На втором этапе было выполнено сравнение производительности PostgreSQL в Ubuntu и Windows при одинаковых параметрах. По результатам трёх прогонов Ubuntu показала средние значения **≈ 4389 TPM** и **≈ 1876 NOPM**, а Windows 11 — **≈ 2263 TPM** и **≈ 967 NOPM**. Таким образом, Ubuntu оказалась примерно в **1.9 раза производительнее Windows** и показала более стабильное поведение под нагрузкой.

На третьем этапе была проведена оптимизация параметров виртуальной машины на базе Ubuntu. При увеличении количества vCPU с **1 до 2** производительность заметно выросла: TPM увеличился с **≈ 3058** до **≈ 4389**, а NOPM — с **≈ 1295** до **≈ 1876**. Дальнейшее увеличение до **4 vCPU** дало небольшой прирост, что указывает на влияние ограничений хостовой машины и дисковой подсистемы.

Также было исследовано влияние объёма RAM. При увеличении памяти с **2 ГБ до 4 ГБ** производительность PostgreSQL заметно повысилась, однако при переходе к **6 ГБ** прирост стал менее выраженным. Это показывает, что после 4 ГБ основным ограничивающим фактором становится уже не оперативная память, а дисковые операции и ресурсы хостовой системы.

Поставленная цель работы была достигнута: была исследована производительность PostgreSQL в разных гостевых ОС и при разных конфигурациях ВМ. По итогам работы для развертывания PostgreSQL под нагрузкой рекомендуется использовать **Ubuntu 24.04 LTS**. Оптимальной конфигурацией для данной хостовой машины является **2 vCPU, 4 ГБ RAM и 40 ГБ дискового пространства**, так как она обеспечивает хороший уровень производительности и не перегружает хостовую Windows 10.
